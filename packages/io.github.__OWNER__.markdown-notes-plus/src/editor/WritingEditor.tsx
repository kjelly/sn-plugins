...
export function WritingEditor({ value, readOnly, onChange, onDeleteTask, command, onCapabilityChange, onLosslessFallback }: WritingEditorProps) {
  ...
  const reportCapability = (result: WritingRoundTripResult) => {
    const next = result.editable;
    if (capabilityRef.current === next) return;
  ...
  useEffect(() => {
    ...
    editor.create().then(() => {
      ...
      const proof = assessWritingRoundTrip(value, serialized);
      capabilityRef.current = proof.editable;
      onCapabilityChangeRef.current?.(proof);
      editor.action((ctx) => ctx.get(editorViewCtx).setProps({ editable: () => !readOnly }));
      if (valueRef.current !== value) {
        const origin = gate.current.suppressExternalUpdate(generation);
        if (origin) editor.action((ctx) => replaceAllWithOrigin(ctx, valueRef.current, origin));
      }
      applyPendingCommand();
  ...
  useEffect(() => {
    ...
    const origin = gate.current.suppressExternalUpdate(generationRef.current);
    if (origin) editor.action((ctx) => replaceAllWithOrigin(ctx, value, origin));
    const serialized = editor.action((ctx) => ctx.get(serializerCtx)(ctx.get(editorViewCtx).state.doc));
    reportCapability(assessWritingRoundTrip(value, serialized));
  }, [value]);
